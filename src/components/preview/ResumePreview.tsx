...
import type { Section, TemplateId } from '../../types'

const PAGE_SIZE_IN: Record<'letter' | 'a4', { w: number; h: number }> = {
  letter: { w: 8.5, h: 11 },
  a4: { w: 8.27, h: 11.69 },
}
...
function ContactLine({ template, accentColor, atsMode }: { template: TemplateId; accentColor: string; atsMode: boolean }) {
...
function Header({ template, accentColor, atsMode }: { template: TemplateId; accentColor: string; atsMode: boolean }) {
  const { resume } = useResume()
  const { contact } = resume
    ...
    return (
      <header className="mb-5 text-center">
        ...
        <div className="mt-2 border-t border-slate-200 pt-2">
          <ContactLine template={template} accentColor={accentColor} atsMode={atsMode} />
        </div>
      </header>
    )
  }
...
export function ResumePreview({ viewAsPages }: ResumePreviewProps) {
  ...
  return (
    <div ref={outerRef} className="w-full">
      ...
      >
        <div id="resume-page" ref={pageRef} className="bg-white text-slate-800 shadow-2xl shadow-black/40" style={pageStyle}>
          <Header template={format.template} accentColor={effectiveAccent} atsMode={format.atsMode} />
